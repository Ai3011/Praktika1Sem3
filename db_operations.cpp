...
void where_select(string& line, int& i, string& logi, List<string>& posl, List<List<string>>& old_formed, List<bool>& cmp){
    while (logi != ""){
            ...
            if (posl.find(first) == -1) {
                first = unwrap_string(first);
            }
            if (posl.find(second) == -1) {
                second = unwrap_string(second);
            }
            ...
            for (int j = 0; j < cmp.length; j++){
                if (logi == "AND") {
                    cmp_res.push(cmp[j] && cmp_new[j]);
                }else if (logi == "OR") {
                    cmp_res.push(cmp[j] || cmp_new[j]);
                }else {
                    throw runtime_error("Invalid logick");
                }
            }
...
void insert(string& line, int& i, HashTable<List<string>>& tables, string& schem_name, int& limit){
    ...
    if (tables.Get(table_name).length < values.length) {
        throw runtime_error("Too many argumets");
    }

    int pk = readPrimaryKeyValue(dirPath/schem_name/table_name, table_name);
    if (pk + 1 > limit) {
        throw runtime_error("Out of limit");
    }
...
void deleting(string& line, int& i, HashTable<List<string>>& tables, string& schem_name){
    ...
    if (take_string(line, i) == "WHERE") {
        where_select(line, i, logi, headers, table, cmp);
    }
